...
//#define H264_MB_TO_SLICE_MAP_ADDR   0x8000CA00
...
*/
// *******************************
//   Display structure (0x8000 C800)
// *******************************
/* global register */
typedef struct _AITS_H264DEC_CTL{
    AIT_REG_B   FRAME_CTL_REG;					//0x00
...
#define SET_NEW_FRAME_START(reg, new_frame_start) { (reg = reg | new_frame_start); }
    ...
    AIT_REG_W   HOST_INT_ENABLE;				//0x08
    ...
    AIT_REG_W   HOST_INT_STATUS;				//0x0A
...
/* reference frame start address : 0x8000C810 */
typedef struct _AITS_H264DEC_REF{
    ...
    AIT_REG_D   Y_SIZE;                 /* D_V1 = 16bits, PYTHON = 20bits */
} AITS_H264DEC_REF, *AITPS_H264DEC_REF;

/* frame reconstruct register address : 0x8000C860 */
typedef struct _AITS_H264DEC_REC{
    AIT_REG_D   REC_FRAME_ADDR;         	//0x0060
    AIT_REG_B   RING_BUF_SIZE;			    //0x0064
    AIT_REG_B   RESERVED1;
    AIT_REG_B   RESERVED2;
    AIT_REG_B   RESERVED3;
    ...
    AIT_REG_B   _x0070[616];                //0x0070
    AIT_REG_D   RECBD_Y_LOW;                //0x02D8
    AIT_REG_D   RECBD_Y_HIGH;               //0x02DC
    AIT_REG_D   RECBD_CBR_LOW;              //0x02E0
    AIT_REG_D   RECBD_CBR_HIGH;             //0x02E4
    AIT_REG_D   RECBD_CR_LOW;               //0x02E8
    AIT_REG_D   RECBD_CR_HIGH;              //0x02EC
    AIT_REG_D   REC_CBR_ADDR;               //0x02F0
    AIT_REG_D   REC_CR_ADDR;                //0x02F4
    AIT_REG_D   REC_UPROW_ADDR;             //0x02F8
    AIT_REG_W   ENC_TIMEOUT_COUNT;          //0x02FC
    AIT_REG_B   ENC_TIMEOUT_MB_X_OFST;      //0x02FE
    AIT_REG_B   ENC_TIMEOUT_MB_Y_OFST;      //0x02FF
    #endif
} AITS_H264DEC_REC, *AITPS_H264DEC_REC;

/* deblocking register address : 0x8000C870 */
typedef struct _AITS_H264DEC_DBLK{
    AIT_REG_D   SOURCE_ADDR;			//0x070
    AIT_REG_D   DEST_ADDR;			    //0x074
    AIT_REG_B   SOURCE_SIZE_IN_MBROWS;  //0x078
    AIT_REG_B   RESERVED3;
    AIT_REG_B   DEBLOCKING_CTRL;		//0x07A
    ...
    AIT_REG_D   DBLK_PARAMS_BUF_ADDR;		//0x07C
    #if (CHIP == VSN_V2) || (CHIP == VSN_V3)
    AIT_REG_B                   _x080[0x258];
    AIT_REG_D   DBLK_OUT_Y_BD_LOW;          //0x2D8
    AIT_REG_D   DBLK_OUT_Y_BD_HIGH;         //0x2DC
    AIT_REG_D   DBLK_OUT_CBR_BD_LOW;        //0x2E0
    AIT_REG_D   DBLK_OUT_CBR_BD_HIGH;       //0x2E4
    AIT_REG_D   DBLK_OUT_CR_BD_LOW;         //0x2E8
    AIT_REG_D   DBLK_OUT_CR_BD_HIGH;        //0x2EC
    AIT_REG_D   DBLK_OUT_CBR_ST;            //0x2F0
    AIT_REG_D   DBLK_OUT_CR_ST;             //0x2F4
    AIT_REG_D   DBLK_UP_ROW_ST;             //0x2F8
    AIT_REG_W   H264_TIME_OUT_LIMIT;        //0x2FC
    #endif
} AITS_H264DEC_DBLK, *AITPS_H264DEC_DBLK;

/* VLD register address : 0x8000C880 */
typedef struct _AITS_H264DEC_VLD{
    AIT_REG_W   PICTURE_SIZE_IN_MBS;		//0x00
    AIT_REG_B   MB_WIDTH;			//0x02
    AIT_REG_B   RESERVED5;
    AIT_REG_B   MB_HEIGHT;			//0x04
    AIT_REG_B   RESERVED6;
    AIT_REG_B   ALPHA_OFFSET_DIV2;		//0x06
    AIT_REG_B   BETA_OFFSET_DIV2;		//0x07
    AIT_REG_B   QP;				//0x08
    AIT_REG_B   CHROMA_QP_INDEX_OFFSET;		//0x09
        /*-DEFINE-----------------------------------------------------*/
        #define H264_CB_QP_IDX_OFST_MASK            0x1F
        #define H264_CB_QP_IDX_OFST(_a)             (_a & H264_CB_QP_IDX_OFST_MASK)
        /*------------------------------------------------------------*/
    AIT_REG_B   DATA_INFO_REG;			//0x0A
    ...
    AIT_REG_B   FRAME_DATA_INFO_REG;		//0x0B
    ...
    AIT_REG_W   START_MB_NR;			//0x0C
    AIT_REG_B   START_MB_XPOS;			//0x0E
    AIT_REG_B   RESERVED8;
    AIT_REG_B   START_MB_YPOS;			//0x10
    AIT_REG_B   RESERVED9;

    //! CHECK REG SPEC P.5 & P.31, Can I combine VLD_CTRL_REG1 & VLD_CTRL_REG2 into one word ?
    AIT_REG_B   VLD_CTL_REG;			//0x12
    ...
    AIT_REG_D   BS_START_ADDR;			//0x14
    AIT_REG_D   BS_LENGTH_MINUS1;		//0x18
    AIT_REG_D   VLD_RINGBUF_LOWERBOUND_ADDR;	//0x1C
    AIT_REG_D   VLD_RINGBUF_HIGHERBOUND_ADDR;	//0x20
    AIT_REG_W   CURRENT_MB_NR;			//0x24
    AIT_REG_B   SLICE_STATUS;       		//0x26	/* read-only */
    AIT_REG_B   RESERVED10;
    AIT_REG_D   BS_SHIFT_BITCOUNT;  		//0x28	/* RESERVED | 3rd byte | 2nd byte | 1st byte */
    AIT_REG_D   SEGMENT_LENGTH;			//0x2C
    AIT_REG_B   MB_STATUS[50/*396 / 8*/]; //0x30
                #define GET_MB_VALID(VLD, i) (((VLD->MB_STATUS[i/8]) >> (i%8)) & 0x1)
	#if (CHIP == P_V2) || (CHIP == VSN_V2) || (CHIP == VSN_V3)
    AIT_REG_B   _x62[6];                //0x62
    AIT_REG_B   HIGH_PROF_REG_1;        //0x68
        /*-DEFINE-----------------------------------------------------*/
        ...
        /*------------------------------------------------------------*/
    AIT_REG_B   HIGH_PROF_REG_2;        //0x69
        /*-DEFINE-----------------------------------------------------*/
        #if (CHIP == VSN_V2) || (CHIP == VSN_V3)
        ...
        #endif
        #if (CHIP == P_V2)
        #define H264_NUM_REF_FRAME_MASK     0x03
        #endif
        /*------------------------------------------------------------*/
    AIT_REG_B   HIGH_PROF_REG_3;        //0x6A
    #if (CHIP == VSN_V2) || (CHIP == VSN_V3)
    AIT_REG_B   HIGH_PROF_REG_4;        //0x6B
        /*-DEFINE-----------------------------------------------------*/
        ...
        /*------------------------------------------------------------*/
    AIT_REG_B   HIGH_PROF_REG_5;        //0x6C
        /*-DEFINE-----------------------------------------------------*/
...
// Parse EP3 address : 0x8000C8F0
typedef struct _AITS_H264DEC_PARSE_EP3{
    AIT_REG_D   BS_SRC_ADDR;			//0xC8F0
    AIT_REG_D	BS_SRC_BITCOUNT;		//0xC8F4
    AIT_REG_D   BS_DST_ADDR;			//0xC8F8
    AIT_REG_D	BS_DST_BITCOUNT;		//0xC8FC  /*Read Only*/
    AIT_REG_B	_xC900[0xF0];
    AIT_REG_D   BS_SRC_LOWBD_ADDR;		//0xC9F0
    AIT_REG_D   BS_SRC_HIGHBD_ADDR;		//0xC9F4
    AIT_REG_D   BS_DST_LOWBD_ADDR;		//0xC9F8
    AIT_REG_D   BS_DST_HIGHBD_ADDR;		//0xC9FC
...
/* SW VLD MB register buffer 0 address : 0x8000C900*/
/* SW VLD MB register buffer 1 address : 0x8000C980*/
typedef struct _AITS_H264DEC_MB_DATA{
    AIT_REG_B   MB_TYPE;			//0x00
    ...
    AIT_REG_W   MB_SLICE_NR;			//0x04
    AIT_REG_W   MB_NR;				//0x06
    AIT_REG_B   MB_XPOS;			//0x08
    AIT_REG_B   RESERVED10;
    AIT_REG_B   MB_YPOS;			//0x0A
    AIT_REG_B   RESERVED11;
    AIT_REG_B   INTRA_MB_PRED_MODE;		//0x0C
    ...
    AIT_REG_B   INTRA_4X4_PRED_MODE[16 / 2];	//0x0E
...
#define H264DEC_REC_UPROW_ADDR               (H264REC_BASE + (MMP_ULONG)(&(((AITPS_H264DEC_REC)0)->REC_UPROW_ADDR                )))
...
#if (CHIP == VSN_V2) || (CHIP == VSN_V3)
#define H264_DBLK_OUT_CBR_ST                 (H264DBLK_BASE + (MMP_ULONG)(&(((AITPS_H264DEC_DBLK)0)->DBLK_OUT_CBR_ST             )))
#define H264_DBLK_OUT_CR_ST                  (H264DBLK_BASE + (MMP_ULONG)(&(((AITPS_H264DEC_DBLK)0)->DBLK_OUT_CR_ST              )))
#define H264_DBLK_UP_ROW_ST                  (H264DBLK_BASE + (MMP_ULONG)(&(((AITPS_H264DEC_DBLK)0)->DBLK_UP_ROW_ST              )))
#endif
#if (CHIP == P_V2)
