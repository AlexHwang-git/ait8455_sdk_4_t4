...
static MMP_USHORT gI2CM_SPEED_CONTROL[MMPF_I2CM_MAX_SPEED_COUNT] = {0, 0, 100, 250, 400, 550, 700, 850, 1000};
...
//static MMP_BOOL m_bI2cmSemInitial = MMP_FALSE;
static MMPF_OS_SEMID  gI2cmSemID[MMPF_I2CM_MAX_COUNT];  
...
//*----------------------------------------------------------------------------
MMP_ERR MMPF_I2c_Start(MMPF_I2CM_ATTRIBUTE *uI2cmAttribute)
{
...
//            Control Pin : P11 -> SCL (O), P8 -> SDA (I/O)
//*----------------------------------------------------------------------------
MMP_ERR MMPF_I2c_Stop(MMPF_I2CM_ATTRIBUTE *uI2cmAttribute)
{
...
//            Control Pin : P11 -> SCL (O), P8 -> SDA (I/O)
//*----------------------------------------------------------------------------
MMP_ERR MMPF_I2c_WriteData(MMPF_I2CM_ATTRIBUTE *uI2cmAttribute, MMP_UBYTE ubData)
{
    MMP_ULONG   i;
    MMP_UBYTE	cShift;
...
//            Control Pin : P11 -> SCL (O), P8 -> SDA (I/O)
//*----------------------------------------------------------------------------
MMP_UBYTE  MMPF_I2c_ReadData(MMPF_I2CM_ATTRIBUTE *uI2cmAttribute)
{
    int i;
...
MMP_ERR  MMPF_I2cm_Initialize(MMPF_I2CM_ATTRIBUTE *uI2cmAttribute)
{
	...
	if (uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW_SENSOR) {
		...
		if (uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_0) {    //HW I2C
            pGBL->GBL_IO_CTL1 |= (GBL_I2C_1_PAD_EN|GBL_I2C_0_PAD_EN);
		...
		pI2CM->I2CMS[uI2cmAttribute->uI2cmID].I2CM_DATA_HOLD_CNT = (usSckDiv>>3);  //MUST not > (usSckDiv/2). Sugguest as (usSckDiv/4)~(usSckDiv/8)
	    pI2CM->I2CMS[uI2cmAttribute->uI2cmID].I2CM_CMDSET_WAIT_CNT = 20;//((usSckDiv>>3) - 1); // Wait N+1 i2cm_clk for next I2C cmd set
...
MMP_ERR	MMPF_I2cm_WriteReg(MMPF_I2CM_ATTRIBUTE* uI2cmAttribute, MMP_USHORT usReg, MMP_USHORT usData)
{
	...
	if ((uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW)||(uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW_SENSOR)) {
        ...
        if (pVIF->VIF_SENSR_CLK_FREQ[vid] < VIF_SENSR_CLK_PLL_D10) {
        }
        else if (pVIF->VIF_SENSR_CLK_FREQ[vid] < VIF_SENSR_CLK_PLL_D14) {
            RTNA_WAIT_US(1);
        }
        else {
            RTNA_WAIT_US(2);
...
// descript : I2C Signal Register Word Reading
//*----------------------------------------------------------------------------
MMP_ERR	MMPF_I2cm_ReadReg(MMPF_I2CM_ATTRIBUTE* uI2cmAttribute, MMP_USHORT usReg, MMP_USHORT *usData)
{
	AITPS_I2CM  pI2CM = AITC_BASE_I2CM;
	MMP_UBYTE	semStatus = 0xFF;
	MMP_ULONG	ulI2cmTimeOut = 0xfff;
...
//	MMP_UBYTE	semStatus = 0xFF;
	...
	if ((uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW)||(uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW_SENSOR)) {
		...
		while (pI2CM->I2CMS[uI2cmAttribute->uI2cmID].I2CM_CTL & I2CM_MASTER_EN){
			ulI2cmTimeOut--;
			if(ulI2cmTimeOut==0){
				RTNA_DBG_Str(0, "I2C time out ws\r\n");
			}
		}
...
MMP_ERR MMPF_I2cm_WriteBurstData(MMPF_I2CM_ATTRIBUTE* uI2cmAttribute, MMP_USHORT usReg, MMP_UBYTE *usData, MMP_UBYTE usDataCnt)
{
	...
	if ((uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW)||(uI2cmAttribute->uI2cmID == MMPF_I2CM_ID_SW_SENSOR)) {
        ...
        if((usDataCnt * (m_bDataLen[uI2cmAttribute->uI2cmID] >> 3) + 3) > I2CM_TX_FIFO_DEPTH){
            // 3 for the case with 16bit register address and one byte for byte count
        ...
        pI2CM->I2CMS[uI2cmAttribute->uI2cmID].I2CM_DMA_CTL &= ~((I2CM_DMA_RX_MODE) | I2CM_DMA_TX_MODE);
		pI2CM->I2CMS[uI2cmAttribute->uI2cmID].I2CM_INT_CPU_SR = I2CM_SLAVE_NO_ACK | I2CM_TX_DONE;
		pI2CM->I2CMS[uI2cmAttribute->uI2cmID].I2CM_SLAV_ADDR &= ~(I2CM_READ_MODE | I2CM_WRITE_MODE);
