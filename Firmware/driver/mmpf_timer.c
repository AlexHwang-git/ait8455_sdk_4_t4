...
//
//==============================================================================
static MMP_BOOL  gbTimerEnabled[MMPF_TIMER_2+1];
...
static TimerCallBackFunc *TimerCallBacks[3] = {NULL, NULL, NULL};
...
MMP_ERR	   MMPF_Timer_EnableInterrupt(MMPF_TIMER_ID id, MMP_BOOL bEnable)
{
	...
	static MMP_UBYTE intEnableCount[3] = {0,0,0}; // FH
...
MMP_ERR MMPF_Timer_OpenUs(MMPF_TIMER_ID id, MMP_ULONG ulUs, TimerCallBackFunc *Func)
{
	...
	#if (CHIP == VSN_V2)
	MMPF_PLL_GetCPUFreq(&cpuFreq);
	#endif
	#if (CHIP == VSN_V3) //In VSN_V3, Timer clock is produced by group 0
	MMPF_PLL_GetGroupFreq(0, &cpuFreq);
	#endif
	cpuFreq /= 1000;
