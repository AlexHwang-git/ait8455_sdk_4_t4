...
SENSOR_I2CM_ID, SENSOR_I2C_ADDR_OV4688, 16, 8, 0, MMP_FALSE, MMP_FALSE, MMP_FALSE,MMP_FALSE, 0, 0, 1, MMPF_I2CM_SPEED_HW_250K, NULL, NULL
#endif
...
const ISP_UINT8 Sensor_IQ_CompressedText[] = {
    ...
    #endif
    #if (CHIP == VSN_V2)
    //#include "isp_8451_iq_data_v1_OV4688.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170630_Mina.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170703_Mina.xls.ciq.txt"
    #include "isp_8453_iq_data_v2_OV4688_20180426_James.xls.ciq.txt"
    #endif
    #if (CHIP == VSN_V3) // IQ table : V3 TBD 
    #if ISP_GNR_OFF==0
    //#include "isp_8453_iq_data_v2_OV4688.xls.ciq.txt" 
    //#include "isp_8453_iq_data_v2_OV4689_20170413_Daniel.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170630_Mina.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170703_Mina.xls.ciq.txt"
    #include "isp_8453_iq_data_v2_OV4688_20180426_James.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_test_1.xls.ciq.txt"
    #else
    //#include "isp_8453_iq_data_v2_OV4688.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4689_20170413_Daniel.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170630_Mina.xls.ciq.txt"
...
const ISP_UINT8 Sensor_IQ_CompressedText1[] = {
    #if (CHIP == P_V2)
    // Undefine!
    #endif
    #if (CHIP == VSN_V2)
    ...
    //#include "isp_8453_iq_data_v2_OV4688_20170630_Mina.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170703_Mina.xls.ciq.txt"
    #include "isp_8453_iq_data_v2_OV4688_20200703_James.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_test_2.xls.ciq.txt"
    #else
    //#include "isp_8453_iq_data_v2_OV4688.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4689_20170413_Daniel.xls.ciq.txt"
    //#include "isp_8453_iq_data_v2_OV4688_20170630_Mina.xls.ciq.txt"
...
const ISP_UINT8 Sensor_IQ_CompressedText2[] = {
    #if (CHIP == P_V2)
    // Undefine!
    #endif
    #if (CHIP == VSN_V2)
...
MMP_ULONG g_SNR_4688_LineCntPerSec = 1;

void SNR_OV4688_InitSensor_Customer(void);
void SNR_OV4688_SetSensorResolution_Customer(ISP_SENSOR_RESOL res);
void MMPF_Sensor_SetSensorGain_OV4688(MMP_ULONG gain);
void MMPF_Sensor_SetSensorShutter_OV4688(MMP_ULONG shutter, MMP_ULONG vsync);

#if SUPPORT_AUTO_FOCUS
void MOTOR_OV4688_VCM_InitMotor_Customer(void);
void MOTOR_OV4688_VCM_SetAFPosition_Customer(MMP_USHORT af_pos);
void MOTOR_OV4688_VCM_SetAFHomePosition_Customer(void);
#endif

#define  SENSOR_WIDTH_16_9_BINNING  SENSOR_16_9_W_60FPS // 1280
#define  SENSOR_HEIGHT_16_9_BINNING SENSOR_16_9_H_60FPS // 720

#define  SENSOR_WIDTH_16_9  2560//1920 
#if SENSOR_IN_H > SENSOR_16_9_H
#define  SENSOR_HEIGHT_16_9 (SENSOR_IN_H)
#else
#define  SENSOR_HEIGHT_16_9 1440//1088 //(1080) 
#endif

#define RAZER_REGION   (1)		//1:For Asia, 0:For Europen	

MMP_USHORT gsSensorWidthByFOV ;
MMP_USHORT gsSensorHeightByFOV ;
MMP_UBYTE  gbZoomFactor = 10;	
...
//------------------------------------------------------------------------------
//  Function    : OV4688_Sensor_PreviewMode
//  Description :
//------------------------------------------------------------------------------
static void OV4688_Sensor_PreviewMode(MMP_USHORT usPreviewmode)
{
	...
	case 0:
	 ...
	 ISP_IF_IQ_SetSysMode(RAZER_REGION);	//ISP_IF_IQ_SetSysMode(1); //Steven NOTE:please reference excel define: 0=capture, 1=preview, 2=video....
        dbg_printf(0,"gsCurFrameRatex10 : %d\r\n",gsCurFrameRatex10[0]);
        SNR_OV4688_SetSensorResolution_Customer(ISP_SENSOR_RESOL_800x600);
	...
	case 1:
		...
		case SENSOR_VIDEO_MODE:
            ...
            SNR_OV4688_SetSensorResolution_Customer(ISP_SENSOR_RESOL_1920x1080);
			//ISP_IF_3A_Control(ISP_3A_ENABLE);
			gsSensorFunction->MMPF_Sensor_Set3AStatus(MMP_TRUE);
			break;
 			//set color transform 16 ~ 235
	...
	case 2:
		dbg_printf(0,"->MJ 1280x720 60fps\r\n");
		switch (gsSensorMode) {
		#if defined(VIDEO_AMR_R_FW)||defined(VIDEO_AAC_R_FW)||(VIDEO_R_EN)
		case SENSOR_VIDEO_MODE:
			//RTNA_DBG_Str(3, "SENSOR_VIDEO_MODE\r\n");
            ...
            SNR_OV4688_SetSensorResolution_Customer(ISP_SENSOR_RESOL_1280x720);
			//ISP_IF_3A_Control(ISP_3A_ENABLE);
			gsSensorFunction->MMPF_Sensor_Set3AStatus(MMP_TRUE);
			break;
 			//set color transform 16 ~ 235
...
MMP_BOOL bMotoInitEn = 1;
ISP_UINT32 Sensor_IQ_TestText;
MMP_USHORT gsDev_ID = 0;

//------------------------------------------------------------------------------
//  Function    : MMPF_Sensor_Initialize
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_Initialize_OV4688(void)
{
	...
	MMPF_PIO_EnableGpioMode(CAM_PWR_EN, MMP_TRUE);			//CAM_PWR_EN		45
    ...
    gsSensorWidthByFOV = SENSOR_WIDTH_16_9 ;  // 2560
    gsSensorHeightByFOV = SENSOR_HEIGHT_16_9; // 1440
	/*
    gsSensorWidthByFOV = VIFByFOVTable[gbZoomFactor][0];
    gsSensorHeightByFOV = VIFByFOVTable[gbZoomFactor][1];
	*/
	...
	SNR_OV4688_InitSensor_Customer();
    ...
    // MJPEG 1280x720 60fps
    //if( (gsVideoFormat == PCAM_USB_VIDEO_FORMAT_MJPEG) && 
    //    ((cur_res->res_w == 1280) && (cur_res->res_h == 720)) &&(max_ep_fps > 300))
    if( max_ep_fps > 300)
    {
	...
	if(bMotoInitEn)
	{
	    MOTOR_OV4688_VCM_InitMotor_Customer();
		bMotoInitEn = 0;
	}
#endif
...
//------------------------------------------------------------------------------
//  Function    : MMPF_Sensor_PowerDown_OV4688
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_PowerDown_OV4688(void)
{
	AITPS_VIF  pVIF = AITC_BASE_VIF;
	AITPS_GPIO  pGPIO = AITC_BASE_GPIO;
	...
	//MMPF_MOTOR_WriteI2C(0xc2, 0x00);		//Power On to floating position
	...
	#if	LED_ENABLE
	MMPF_PIO_SetData(PWR_LED, MMP_FALSE);
	#endif 
#if SUPPORT_LED_DRIVER
	//MMPF_PIO_EnableGpioMode(LED_HWEN, MMP_TRUE);
	//MMPF_PIO_EnableOutputMode(LED_HWEN, MMP_TRUE);
	...
	RTNA_DBG_Str3( "MMPF_Sensor_PowerDown_OV4688() \r\n");
	return  MMP_ERR_NONE;
}

//------------------------------------------------------------------------------
//  Function    : MMPF_Sensor_SetFrameRate_OV4688
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_SetFrameRate_OV4688(MMP_UBYTE ubFrameRate)
{
	return  MMP_ERR_NONE;
}
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR MMPF_Sensor_SetPreviewMode_OV4688(MMP_USHORT usPreviewmode)
{
	OV4688_Sensor_PreviewMode(usPreviewmode);
...
//  Description :
//------------------------------------------------------------------------------
/*static*/ MMP_ERR MMPF_Sensor_ChangeMode_OV4688(MMP_USHORT usPreviewmode, MMP_USHORT usCapturemode)
{
	switch (usPreviewmode) {
	case 0:
	...
	case 1:
		...
		case 0:// SXGA preview, 5M capture
			MMPF_Sensor_SetPreviewMode_OV4688(0);
			// wait 3 frame ...
		   // MMPF_Sensor_WaitFrame(5);
			break;
		case 1:
			MMPF_Sensor_SetPreviewMode_OV4688(1);
			// wait 3 frame ...
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_InitializeVIF_OV4688(void)
{
	AITPS_MIPI  pMIPI = AITC_BASE_MIPI;
...
#define	eeprom_I2C_ADDR 0x54	//	0x54  0x50

MMPF_I2CM_ATTRIBUTE m_eeprom_I2cmAttribute = {
SENSOR_I2CM_ID, eeprom_I2C_ADDR, 16, 8, 0, MMP_FALSE, MMP_FALSE, MMP_FALSE,
MMP_FALSE, 0, 0, 1, MMPF_I2CM_SPEED_HW_250K, NULL, NULL};


void Read_Truly_EEPROM(ISP_UINT16 usReg, ISP_UINT8 *usData, ISP_UINT16 usDataCnt)
{
	ISP_UINT16 cnt, cnt1;
	ISP_UINT16 data_buff;
	cnt1 = 0;
	for(cnt = 0 ; cnt <= usDataCnt ; cnt++)
	{
		MMPF_I2cm_ReadReg(&m_eeprom_I2cmAttribute, (usReg+cnt),&data_buff);
		//dbg_printf(3, " EEP 0x00  0x%x\r\n",  data_buff);	
		//*(usData + cnt1) = data_buff >> 8;
		//cnt1++;
		*(usData + cnt) = (ISP_UINT8)data_buff;
		//dbg_printf(3, " EEP 0x01 02  0x%x  %x\r\n",  *(usData + cnt1 -1), *(usData + cnt1));	
		//cnt1++;
		
	}
	/*
	ISP_UINT8 Read_cnt, Read_cntAcc;
	Read_cntAcc = 0;
	while(Read_cntAcc < usDataCnt)
	{
		if((usDataCnt - Read_cntAcc) > 255)		
			Read_cnt = 255;
		else		
			Read_cnt = usDataCnt;
		
		MMPF_I2cm_ReadBurstData(&m_eeprom_I2cmAttribute, usReg+Read_cntAcc, usData+Read_cntAcc,Read_cnt);
		Read_cntAcc += Read_cnt;
	}*/
}
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_InitializeISP_OV4688(void)
{
	volatile MMP_BYTE* pISP = (volatile MMP_BYTE*)AITC_BASE_ISP;
	//ISP_UINT8	*eeprom_buff;
	//P_USHORT	print_cnt;
	ISP_UINT16 R_gain,G_gain,B_gain;
	ISP_UINT8 *module_ver = 0;
	ISP_UINT8 print_cnt = 0;
	...
	#if 1	//AlexH
	Read_Truly_EEPROM(0x1525,module_ver,1);
	#else
	*module_ver = 2;
	#endif
	dbg_printf(3, "===================== module_ver: 0x%x =====================\r\n", *module_ver);	
	
	if(*module_ver == 0)
	{
		Sensor_IQ_TestText = (ISP_UINT32)Sensor_IQ_CompressedText;
		IQID = 0x11;
	}
	else if(*module_ver == 1)
	{
		Sensor_IQ_TestText = (ISP_UINT32)Sensor_IQ_CompressedText1;
		IQID = 0x12;
	}
	else {
		Sensor_IQ_TestText = (ISP_UINT32)Sensor_IQ_CompressedText2;
		IQID = 0x13;
	}
	
	//(MMP_BYTE *)SERIAL_NUMBER_READ_BUF_STA = HWID;
	//*(serial_number_dma_buff+23) = HWID;
   ...
   eeprom_buff = (ISP_UINT8 *)CALI_DATA_BUF_STA;
	...
	eeprom_buff = (ISP_UINT8 *)CALI_DATA_BUF_STA;
	#if 0
	for(print_cnt = (1593 + 2816); print_cnt <100 + (1593 + 2816);print_cnt++)
	{
		dbg_printf(3, " I2C 0x00  0x%x\r\n", *(eeprom_buff+print_cnt));		
	}
    #endif
	#if 0
	...
	*(eeprom_buff+0x27) = 0x01;*/
	...
	R_gain = ISP_IF_CALI_GetAWBGainR();
	G_gain = ISP_IF_CALI_GetAWBGainG();
	B_gain = ISP_IF_CALI_GetAWBGainB();
	dbg_printf(3, " RGain3  0x%x  %x  %x\r\n", R_gain, G_gain, B_gain);
	ISP_IF_CALI_SetCS_Bin(0, 5000, 1000, (MMP_USHORT *)(eeprom_buff + 0x638));
	ISP_IF_CALI_SetCS_Bin(1, 3000, 1000, (MMP_USHORT *)(eeprom_buff + 0x1138));	

	
	ISP_IF_CALI_SetLS_Bin(0, 5000, 1000, (MMP_USHORT *)(eeprom_buff + 0x28));
	ISP_IF_CALI_SetLS_Bin(1, 3000, 1000, (MMP_USHORT *)(eeprom_buff + 0xB28));
...
//------------------------------------------------------------------------------
//  Function    : MMPF_Sensor_SetAFPosition_OV4688
//  Description :
// ---------------------------------------
static MMP_ERR MMPF_Sensor_SetAFPosition_OV4688(MMP_UBYTE ubPos)
{
#if SUPPORT_AUTO_FOCUS
	//if (gISPConfig.AutoFocusMode == VENUS_AF_MODE_MANUAL) {
		...
		ISP_IF_AF_SetPos(ubPos, 10/*8 0*/);

        MOTOR_OV4688_VCM_SetAFPosition_Customer(ISP_IF_AF_GetPos(0));
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_DoAWBOperation_OV4688(void)
{
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_DoAEOperation_ST_OV4688(void)
{

    if(ISP_IF_3A_GetSwitch(ISP_3A_ALGO_AE) != 1) return MMP_ERR_NONE;
	
	/*
	if(gISPFrameCnt%30 ==0){
	   RTNA_DBG_Str(0, "\r\n GetPos:");
	    RTNA_DBG_Long(0, ISP_IF_AF_GetPos(10));
	}*/
	
	/*RTNA_DBG_Str(0, "\r\n WDR level1111111111111111111");
	RTNA_DBG_Long(0, ISP_IF_F_GetWDREn());
	RTNA_DBG_Long(0, ISP_IF_F_GetWDR());
	RTNA_DBG_Str(0, "\r\n");*/
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_DoAEOperation_END_OV4688(void)
{
	gISPFrameCnt++;
    if(ISP_IF_3A_GetSwitch(ISP_3A_ALGO_AE) != 1) return MMP_ERR_NONE;
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_DoAFOperation_OV4688(void)
{
#if SUPPORT_AUTO_FOCUS
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_DoAFOperation_FrameStart_OV4688(void)
{
#if SUPPORT_AUTO_FOCUS //&& (AF_EN!=255)
	if(gbAutoFocus == MMP_TRUE){
		//VR_AF_GetAcc();
...
//------------------------------------------------------------------------------
//  Function    : MMPF_Sensor_DoIQOperation_OV4688
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_Sensor_DoIQOperation_OV4688(void)
{
   MMP_USHORT i,w[2] = {0,0} ,h[2] = {0,0}; // endpoint 0 & 1
    ...
    colortemp = ISP_IF_AWB_GetColorTemp();


/*	
	if(colortemp < 3500){
	
			ISP_IF_IQ_SetOpr(0x708c,1,0x90);
			ISP_IF_IQ_SetOpr(0x708d,1,0x80);
			ISP_IF_IQ_SetOpr(0x708e,1,0x80);
     }
     else{
	
			ISP_IF_IQ_SetOpr(0x708c,1,0x80);
			ISP_IF_IQ_SetOpr(0x708d,1,0x80);
			ISP_IF_IQ_SetOpr(0x708e,1,0x80);
     }
...
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR  MMPF_SetLightFreq_OV4688(MMP_USHORT usMode)
{
/*	RTNA_DBG_Str(3, "MMPF_SetLightFreq_OV4688 : ");
...
static MMP_ERR  MMPF_SetStableState_OV4688(MMP_BOOL bStableState)
{
	return  MMP_ERR_NONE;
}

MMP_ERR MMPF_SetHistEQState_OV4688(MMP_BOOL bHistEQState)
{

	return  MMP_ERR_NONE;
}

static MMP_ERR  MMPF_Sensor_SetReg_OV4688(MMP_USHORT usAddr, MMP_USHORT usData)
{
	MMPF_I2cm_WriteReg(&m_snr_I2cmAttribute, usAddr, usData);
	return  MMP_ERR_NONE;
}

static MMP_ERR  MMPF_Sensor_GetReg_OV4688(MMP_USHORT usAddr, MMP_USHORT *usData)
{
	*usData = 0xFF;
...
static MMP_ERR  MMPF_Sensor_SetImageScene_OV4688(MMP_USHORT imgSceneID)
{
/*
typedef enum {
...
*/
	...
	RTNA_DBG_Str(3, "MMPF_SetImageScene_OV4688 : ");
...
static MMP_ERR  MMPF_Sensor_SetAEMode_OV4688(MMP_UBYTE ubAEMode, MMP_UBYTE ubISOMode)
{
/*
typedef enum {
	AE_MODE_AUTO						= 0,
...
*/
	RTNA_DBG_Str(3, "MMPF_Sensor_SetAEMode_OV4688 : ");
...
static MMP_ERR  MMPF_Sensor_SetAFMode_OV4688(MMP_UBYTE ubAFMode)
{
/*
typedef enum {
...
*/
	RTNA_DBG_Str(3, "MMPF_Sensor_SetAFMode_OV4688 : ");
...
// description :                                                        //
//======================================================================//
static MMP_ERR  MMPF_Sensor_SetImageEffect_OV4688(MMP_USHORT imgEffectID)
{
/*
typedef enum {
...
*/
	...
	RTNA_DBG_Str(3, "MMPF_Sensor_SetImageEffect_OV4688 : ");
...
static MMP_ERR  MMPF_Sensor_SetAWBType_OV4688(MMP_UBYTE ubType)
{
#if 0
	RTNA_DBG_Str(3, "MMPF_Sensor_SetAWBType_OV4688 : ");
...
static MMP_ERR  MMPF_Sensor_SetContrast_OV4688(MMP_SHORT ubLevel)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_Contrast_OV4688 : ");
...
static MMP_ERR  MMPF_Sensor_SetSaturation_OV4688(MMP_SHORT ubLevel)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_SetSaturation_OV4688 : ");
	RTNA_DBG_Byte(3, ubLevel);
	RTNA_DBG_Str(3, "\r\n");
...
static MMP_ERR  MMPF_Sensor_SetSharpness_OV4688(MMP_SHORT ubLevel)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_SetSharpness_OV4688 : ");
	RTNA_DBG_Byte(3, ubLevel);
	RTNA_DBG_Str(3, "\r\n");
...
static MMP_ERR  MMPF_Sensor_SetHue_OV4688(MMP_SHORT ubLevel)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_SetHue_OV4688 : ");
	RTNA_DBG_Byte(3, ubLevel);
	RTNA_DBG_Str(3, "\r\n");
...
static MMP_ERR  MMPF_Sensor_SetGamma_OV4688(MMP_SHORT ubLevel)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_SetGamma_OV4688 : ");
	RTNA_DBG_Byte(3, ubLevel);
	RTNA_DBG_Str(3, "\r\n");
...
static MMP_ERR  MMPF_Sensor_SetBacklight_OV4688(MMP_UBYTE ubLevel)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_SetBacklight_OV4688 : ");
	RTNA_DBG_Byte(3, ubLevel);
	RTNA_DBG_Str(3, "\r\n");
...
static void  MMPF_Sensor_SetCaptureISPSetting_OV4688(MMP_UBYTE usCaptureBegin)
{
	AITPS_VIF  pVIF = AITC_BASE_VIF;
...
static void  MMPF_Sensor_SetISPWindow_OV4688(MMP_ULONG ulWidth, MMP_ULONG ulHeight)
{
...
static MMP_UBYTE  MMPF_Sensor_CheckPreviewAbility_OV4688(MMP_USHORT usPreviewmode)
{
	switch(usPreviewmode){
...
static void SetAFWin_OV4688(MMP_USHORT usIndex, MMP_USHORT usPreviewWidth, MMP_USHORT usPreviewHeight,
...
static void SetAFEnable_OV4688(MMP_UBYTE enable)
{
#if SUPPORT_AUTO_FOCUS //&& (AF_EN!=255)
    if (enable) {
...
static MMP_ERR MMPF_Sensor_SetAEEnable_OV4688(MMP_UBYTE bEnable)
{
	/*
	if (bEnable) {
...
static MMP_ERR MMPF_Sensor_SetAWBEnable_OV4688(MMP_UBYTE bEnable)
{
#if 0
	if (bEnable) {
...
static void SetAEsmooth_OV4688(MMP_UBYTE smooth)  {return;}
static MMP_ERR MMPF_Sensor_SetExposureValue_OV4688(MMP_UBYTE ev)
{
	RTNA_DBG_Str(3, "MMPF_Sensor_SetExposureValue_OV4688 : ");
...
static MMP_UBYTE GetAFPosition_OV4688     (void){ return 0;/*volatile MMP_UBYTE *GPIO_BASE_B = (volatile MMP_UBYTE *)0x80009400;  return GPIO_BASE_B[0xB0];*/}


static MMP_UBYTE GetAFEnable_OV4688(void)
{
	//return 0;
	return ISP_IF_AF_GetResult();
}

static MMP_UBYTE GetExposureValue_OV4688  (void){return ISP_IF_AE_GetEV();}
static MMP_UBYTE GetAEsmooth_OV4688       (void){return 0;}

//------------------------------------------------------------------------------
//  Function    : MMPF_Sensor_Set3AStatus
//  Description :
//------------------------------------------------------------------------------
static MMP_ERR	MMPF_Sensor_Set3AStatus_OV4688(MMP_BOOL bEnable)
{
...
static void  MMPF_Sensor_SetColorID_OV4688(MMPF_SENSOR_ROTATE_TYPE RotateType)
{
...
MMPF_SENSOR_FUNCTION  SensorFunction_OV4688 =
{
	MMPF_Sensor_Initialize_OV4688,
	MMPF_Sensor_InitializeVIF_OV4688,
	MMPF_Sensor_InitializeISP_OV4688,
	MMPF_Sensor_PowerDown_OV4688,
	MMPF_Sensor_SetFrameRate_OV4688,
	MMPF_Sensor_ChangeMode_OV4688,
	MMPF_Sensor_ChangePreviewMode_OV4688,
	MMPF_Sensor_SetPreviewMode_OV4688,
	MMPF_Sensor_SetReg_OV4688,
	MMPF_Sensor_GetReg_OV4688,
	MMPF_Sensor_DoAWBOperation_OV4688,
	MMPF_Sensor_DoAEOperation_ST_OV4688,
	MMPF_Sensor_DoAEOperation_END_OV4688,
	MMPF_Sensor_DoAFOperation_OV4688,
	MMPF_Sensor_DoAFOperation_FrameStart_OV4688,
	MMPF_Sensor_DoIQOperation_OV4688,
	MMPF_Sensor_SetImageEffect_OV4688,
	MMPF_SetLightFreq_OV4688,
	MMPF_SetStableState_OV4688,
	MMPF_SetHistEQState_OV4688,
	MMPF_Sensor_SetAFPosition_OV4688,
	SetAFWin_OV4688,
	SetAFEnable_OV4688,
	MMPF_Sensor_SetAEEnable_OV4688,
	MMPF_Sensor_SetAWBEnable_OV4688,
	MMPF_Sensor_SetExposureValue_OV4688,
	SetAEsmooth_OV4688,
	MMPF_Sensor_SetImageScene_OV4688,
	GetAFPosition_OV4688,
	GetAFEnable_OV4688,
	GetExposureValue_OV4688,
	GetAEsmooth_OV4688,
	MMPF_Sensor_SetAWBType_OV4688,
	MMPF_Sensor_SetContrast_OV4688,
	MMPF_Sensor_SetSaturation_OV4688,
	MMPF_Sensor_SetSharpness_OV4688,
	MMPF_Sensor_SetHue_OV4688,
	MMPF_Sensor_SetGamma_OV4688,
 //   MMPF_Sensor_SetBacklight_OV4688,
	MMPF_Sensor_SetAEMode_OV4688,
	MMPF_Sensor_SetAFMode_OV4688,
	MMPF_Sensor_SetCaptureISPSetting_OV4688,
	MMPF_Sensor_SetISPWindow_OV4688,
	MMPF_Sensor_CheckPreviewAbility_OV4688,
	MMPF_Sensor_Set3AStatus_OV4688,
	MMPF_Sensor_SetColorID_OV4688,
	MMPF_Sensor_SetSensorGain_OV4688,
	MMPF_Sensor_SetSensorShutter_OV4688
};

MMP_USHORT SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[] = {
...
MMP_USHORT SNR_OV4688_Reg_1920x1080_2Lane_30P_696M[] = {
	...
	0x481f, 0x40, 
	0x4829, 0x78, 
	0x4837, 0x14,			//;10 for Global MIPI UI 
	0x4b00, 0x2a, 
	0x4b0d, 0x00, 
	0x4d00, 0x04, 
	0x4d01, 0x42, 
...
MMP_USHORT SNR_OV4688_Reg_2560x1440_2Lane_30P_696M[] = {
...
MMP_USHORT SNR_OV4688_Reg_2688x1520_30P[] = {
	...
	0x3000, 0x20,
	0x3002, 0x00,
	0x3018, 0x32, // 72 -> 4 lane, 32 -> 2 lane
	0x3020, 0x93,
	0x3021, 0x03,
	0x3022, 0x01,
	0x3031, 0x0a,
	...
	0x3806, 0x05,
	0x3807, 0xfb,
	0x3808, 0x0a, // H_OUTPUT_SIZE 0xa84 = 2692
	0x3809, 0x84,
	0x380a, 0x05, // V_OUTPUT_SIZE 0x5f4 = 1524
	0x380b, 0xf4,
	0x380c, 0x0a, // 03 TIMING_HTS 0xa14 = 2580
	0x380d, 0x14, // 5c
	0x380e, 0x06, // TIMING_VTS 0x612 = 1554
	0x380f, 0x12,
	0x3810, 0x00,
	0x3811, 0x08,
	0x3812, 0x00,
...
// OV4688_1924x1092_2lane_696Mbps
MMP_USHORT SNR_OV4688_Reg_Init_Customer[] = {
	...
	0x3806, 0x05,
	0x3807, 0x25, //0x1f,
	0x3808, 0x0a, // H_OUTPUT_SIZE 0x784 = 1924
	0x3809, 0x84, // H_OUTPUT_SIZE 0xa84 = 2692
	0x380a, 0x05, // V_OUTPUT_SIZE 0x43c = 1092, 0x444=1100
	0x380b, 0xf4, // V_OUTPUT_SIZE 0x5f4 = 1524
	0x380c, 0x0a, // 03 TIMING_HTS 0xa14 = 2580
	0x380d, 0x14, // 5c
	#else
	0x3800, 0x00,
...
// OV4688_1280x720_2x_Bin
MMP_USHORT SNR_OV4688_Reg_1280x720_60P[] = {

//=======================================
//MCLK=	24.00	Mhz
//Resolution x , y =	1284	x	724
//MIPI Lanes =	2	-Lanes
//SCLK=	120.00	Mhz
//MIPI Data Rate = 	324.00	Mbps
//FPS=	60.08
//HTS=	2584	,	0x0A18
//VTS=	773	,	0x0305
//Tline=	21.53333333	us
//reg0x4837 =	0x32
//=======================================
    ...
    0x3000	,	0x20	,				
    0x3002	,	0x00	,				
    0x3018	,	0x32	,	// 72 set 0x32 for mipi 2-lanes	
    0x3020	,	0x93	,				
    0x3021	,	0x03	,				
    0x3022	,	0x01	,				
    0x3031	,	0x0a	,				
    ...
    0x3806	,	0x05	,				
    0x3807	,	0xD7	,				
    0x3808	,	0x05	,   // H_OUTPUT_SIZE 0x504 = 1284
    0x3809	,	0x04	,				
    0x380A	,	0x02	,   // H_OUTPUT_SIZE 0x2D4 = 724
    0x380B	,	0xD4	,				
    0x380c	,	0x0a	,	//	3	TIMING_HTS 0xa18 = 2584
    0x380d	,	0x18	,	//	5C		
    0x380e	,	0x03	,   //      TIMING_VTS 0x305 = 773
    0x380f	,	0x05	,				
    0x3810	,	0x00	,				
    0x3811	,	0x04	,				
    0x3812	,	0x00	,				
...
void SNR_OV4688_InitSensor_Customer(void) {
	...
	vif_if = ISP_HDM_VIF_IF_MIPI_QUAD;
		...
		switch(ulSENSORCLK){
			...
			case 240000:
				# if 0
				pVIF->VIF_SENSR_CLK_FREQ[vid] = VIF_SENSR_CLK_PLL_D12;
				RTNA_DBG_Str3("240MHz DIV 12\r\n");
				# else
...
#if 0
	// set register..........
	//for (i = 0; i < VR_ARRSIZE(SNR_OV4688_Reg_Init_Customer)/2; i++) {
	for (i = 0; i < VR_ARRSIZE(SNR_OV4688_Reg_2688x1520_2Lane_30P_696M)/2; i++) {
		if(SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[i*2] == 0xFFFF)
		//if(SNR_OV4688_Reg_Init_Customer[i*2] == 0xFFFF)
			//delay_1ms(SNR_OV4688_Reg_Init_Customer[i*2+1]);
			delay_1ms(SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[i*2+1]);
		else
			//gsSensorFunction->MMPF_Sensor_SetReg(SNR_OV4688_Reg_Init_Customer[i*2], SNR_OV4688_Reg_Init_Customer[i*2+1]);
			gsSensorFunction->MMPF_Sensor_SetReg(SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[i*2], SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[i*2+1]);
            if(MMPF_Sensor_GetErr()) {
                return ;
            }

        /*
		//data = gsSensorFunction->MMPF_Sensor_GetReg(SNR_OV4688_Reg_Init_Customer[i*2]) ;
		data = gsSensorFunction->MMPF_Sensor_GetReg(SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[i*2]) ;
		RTNA_DBG_Str3("Reg :");
		//RTNA_DBG_Short3(SNR_OV4688_Reg_Init_Customer[i*2]);
		RTNA_DBG_Short3(SNR_OV4688_Reg_2688x1520_2Lane_30P_696M[i*2]);
		RTNA_DBG_Str3(",");
...
void MMPF_Sensor_SetSensorShutter_OV4688(MMP_ULONG shutter, MMP_ULONG vsync)
{
    ISP_UINT32 new_vsync   = g_SNR_4688_LineCntPerSec * ISP_IF_AE_GetVsync() / ISP_IF_AE_GetVsyncBase();
    ISP_UINT32 new_shutter = g_SNR_4688_LineCntPerSec * ISP_IF_AE_GetShutter() / ISP_IF_AE_GetShutterBase();
    ...
    //dbg_printf(0,"%x %x\r\n", ISP_IF_AE_GetDbgData(0), ISP_IF_AE_GetDbgData(6));
...
void SNR_OV4688_SetSensorAddMode_Customer(ISP_UINT8 enable)
{
...
void SNR_OV4688_SetSensorResolution_Customer(ISP_SENSOR_RESOL res)
{
	...
	case ISP_SENSOR_RESOL_800x600:
        ...
        //for (i = 0; i < VR_ARRSIZE(SNR_OV4688_Reg_Init_Customer)/2; i++) {
        	//gsSensorFunction->MMPF_Sensor_SetReg(SNR_OV4688_Reg_Init_Customer[i*2], SNR_OV4688_Reg_Init_Customer[i*2+1]);
        for (i = 0; i < VR_ARRSIZE(SNR_OV4688_Reg_2560x1440_2Lane_30P_696M)/2; i++) {
		if(SNR_OV4688_Reg_2560x1440_2Lane_30P_696M[i*2] == 0xFFFF)
			delay_1ms(SNR_OV4688_Reg_2560x1440_2Lane_30P_696M[i*2+1]);
		else
        		gsSensorFunction->MMPF_Sensor_SetReg(SNR_OV4688_Reg_2560x1440_2Lane_30P_696M[i*2], SNR_OV4688_Reg_2560x1440_2Lane_30P_696M[i*2+1]);
       ...
       g_SNR_4688_LineCntPerSec = (1510)*30; ///test20120821 +2 for DMA


        ISP_IF_AE_SetShutterBase(g_SNR_4688_LineCntPerSec);
		...
		VIFGrab_H_Length    = SENSOR_WIDTH_16_9 + 4;//VIFByFOVTable[gbZoomFactor][0]+4;		//1920 + 4;
		VIFGrab_V_Start     = 1;
		VIFGrab_V_Length    = SENSOR_HEIGHT_16_9+4;//VIFByFOVTable[gbZoomFactor][1]+4;	//1092; // 1088 (H264 Width 16 alignment) + 4 (ISP)
...
//		ISP_IF_IQ_SetScalerOutputLength(VIFGrab_H_Length, VIFGrab_V_Length, 0 ,0);	//20121130, pan/tile/zoom	
	...
	case ISP_SENSOR_RESOL_1280x720:
		...
		// set register...........
        for (i = 0; i < VR_ARRSIZE(SNR_OV4688_Reg_1280x720_60P)/2; i++) {
		if(SNR_OV4688_Reg_1280x720_60P[i*2] == 0xFFFF)
			delay_1ms(SNR_OV4688_Reg_1280x720_60P[i*2+1]);
		else
        		gsSensorFunction->MMPF_Sensor_SetReg(SNR_OV4688_Reg_1280x720_60P[i*2], SNR_OV4688_Reg_1280x720_60P[i*2+1]);
        }

		// set target fps and corresponding sensor vsync
        ISP_IF_AE_SetMaxSensorFPSx10(60*10);
        g_SNR_4688_LineCntPerSec = (750)*60; // TBD

        ISP_IF_AE_SetShutterBase(g_SNR_4688_LineCntPerSec);

    	// set vif grab range which is fetched to ISP (for lens shading and 3A window calculation)
		VIFGrab_H_Start     = 1; //shift right 4 pixel, sensor start from 2
		VIFGrab_H_Length    = 1280 + 4;
		VIFGrab_V_Start     = 1;
		VIFGrab_V_Length    = 720 + 4;
...
//		ISP_IF_IQ_SetScalerOutputLength(VIFGrab_H_Length, VIFGrab_V_Length, 0 ,0);	//20121130, pan/tile/zoom	
	...
	//	y_offset  = ISP_MAX(((VIFGrab_H_Length * base_y / base_x) - VIFGrab_V_Length)/2, 0);
		...
		dbg_printf(0,"VIFGrab_H_Length,VIFGrab_V_Length=(%d,%d)\r\n",VIFGrab_H_Length,VIFGrab_V_Length);
...
ISP_UINT16	AF_Mar, AF_Inf;
ISP_UINT8	VCM_version = 0;	
void MOTOR_OV4688_VCM_InitMotor_Customer(void)
{
	ISP_UINT8	*eeprom_buff;
...
#else
//	ISP_IF_AF_SetSearchRange(0x18 * 4, 0x78 * 4, 0x18 * 4, 0x78 * 4, 10);
  #if 1 //For BU64297GWZ		ALEXH@20170728
    if(VCM_version == 1 || VCM_version == 2)
   	 	MMPF_MOTOR_WriteI2C(0xc2, 0xee);	//Power On to floating position
    else
		MMPF_MOTOR_WriteI2C(0xc2, 0x00);	//Power On to floating position
    MMPF_MOTOR_WriteI2C(0xd0, 0x85);		//Set Rosonance frequency
    MMPF_MOTOR_WriteI2C(0xc8, 0x02);		//Step mode, 1.0x mode    
    //ISP_IF_AF_SetSearchRange(0x18 * 4, 0x78 * 4, 0x18 * 4, 0x78 * 4, 10);
    //ISP_IF_AF_SetSearchRange(0x1a0, 0x29c, 0x1a0, 0x29c, 10);
	...
	//dbg_printf(3, " ---------------------------- AF_Inf %x  %x %x --------------------------\r\n",  AF_Inf, *(eeprom_buff + 0x1524), *(eeprom_buff + 0x1523));
    ISP_IF_AF_SetSearchRange(AF_Inf,AF_Mar, AF_Inf,AF_Mar , 10);
	////ISP_IF_AF_SetSearchRange(0x2b8, 0x3FF, 0x2b8, 0x3FF, 10);
  #else
    ISP_IF_AF_SetSearchRange(0x08 * 4, 0x78 * 4, 0x08 * 4, 0x78 * 4, 10);
  #endif
    //ISP_IF_AF_SetPos(0x08, 10);
    //if(VCM_version == 1)
		 //ISP_IF_AF_SetPos( AF_Mar , 10);		 
	ISP_IF_AF_SetPos( AF_Inf , 10);
    //else
    //		ISP_IF_AF_SetPos( AF_Inf_def , 10);
   
#endif
	dbg_printf(0, "------- AF_Inf = 0x%x-----AF_Mar = 0x%x--------\r\n",AF_Inf,AF_Mar);
...
void MOTOR_OV4688_VCM_SetAFPosition_Customer(MMP_USHORT af_pos)
{
	ISP_UINT32*	glAccData ;
	MMP_UBYTE printACC_index;
	
	glAccData = ISP_IF_AF_GetAccPtr();
	#if 0		//For Tatung/Razer project new sensor module, VCM driver is "BG64297", ALEXH@20170728 
    //AF_10BIT_INTERFACE (DW9714)
    // =================================
    // SDA = --------+--------+--------
    //       ---11--W|S-987654|3210----
    //          ID      addr     data
	...
	#else
    //AF_10BIT_INTERFACE (BU64297GWZ)
    // =================================
    // SDA = --------+- - - - - - - - +- - - - - - - - |
    //       ---11--W|PSENW2W1W0M D9D8|D7D6D5D4D3D2D1D0|
    //          ID           addr            data
    //
    // addr = (gsAFPos >> 8) & 0x03
    // data = gsAFPos  & 0xff
    ...
    MMPF_MOTOR_WriteI2C(((af_pos >> 8) & 0x03 ) | 0xc0, af_pos & 0xff);
	//dbg_printf(0, "af_pos = 0x%x   5th=%x    15th= %x \r\n   ",af_pos, *(ISP_UINT32*)(glAccData + 4), *(ISP_UINT32*)(glAccData + 14) );
	/*
    if(frm_cnt == 9)
    {
    		
	    for(printACC_index = 0; printACC_index < 20; printACC_index++)
	    	{
	    		 dbg_printf(0, "0x%x     ", *(ISP_UINT32*)(glAccData + printACC_index));
			if( printACC_index == 9 )	 
				dbg_printf(0, "\r\n");
	    	}
		dbg_printf(0, "\r\n");
		...
		frm_cnt++;*/
	#endif
    //dbg_printf(3,"###MOTOR_OV4688_VCM_SetAFPosition_Customer(%d)###\r\n", af_pos);
}
...
void MOTOR_OV4688_VCM_SetAFHomePosition_Customer(void)
{
    ...
    //dbg_printf(3,"###MOTOR_OV4688_VCM_SetAFHomePosition_Customer(%d)###\r\n", af_pos);	
}

#endif


MMPF_SENSOR_FUNCTION  *SensorFunction_Module0 = &SensorFunction_OV4688;
#if (TOTAL_SENSOR_NUMBER >= 2)
MMPF_SENSOR_FUNCTION  *SensorFunction_Module1 = &SensorFunction_OV4688;
#endif

#endif  //BIND_SENSOR_OV4688
#endif
