...
typedef struct {
	...
	// Skip Frames
	ISP_UINT32				SkipCnt;					// AWB skip count
	ISP_UINT32				SkipNum;					// AWB skip number (Skip several DoAWB to prevent from using incorrect Accs)
	...
	ISP_UINT32				CalCTGainB;						// B Gain | 1X: GainBase
	// Custom Parameters
	...
	ISP_UINT16				CusDefaultRgain;
	ISP_UINT16				CusDefaultBgain;
	// Debug Data
	ISP_UINT32				DbgData[64];				// debug data - used to store debug data (will be shown in JPEG)
